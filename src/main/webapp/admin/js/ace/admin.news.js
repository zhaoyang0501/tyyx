jQuery.adminNews = {
					...
					"aoColumnDefs" : [
						{
							...
							'fnRender' : function(oObj, sVal) {
								return"  <button class=\"btn2 btn-info\" onclick=\"$.adminNews.deleteNotice("+oObj.aData.id+")\"><i class=\"icon-trash\"></i> 删除</button>";
							}
						},
					 {
						'aTargets' : [ '_all' ],
		...
		deleteNotice :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/news/delete/"+id,
	        			dataType : "json",
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/news/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
